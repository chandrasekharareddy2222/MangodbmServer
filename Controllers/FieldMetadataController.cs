...
namespace FieldMetadataAPI.Controllers
{
    ...
    public class FieldMetadataController : ControllerBase
    {
        ...
        [HttpGet("check-table-value")]
        public async Task<IActionResult> CheckTableValue(
    [FromQuery] string tableName)
    //[FromQuery] string? keyValue = null)
        {
            if (string.IsNullOrWhiteSpace(tableName))
                return BadRequest("Table name is required");

            var result = await _service.CheckTableValueAsync(tableName);
